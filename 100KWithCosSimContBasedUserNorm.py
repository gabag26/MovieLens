...
ratingsWithOne = data.map(lambda l: l.split()).map(lambda l: (int(l[0]), (float(l[2]), 1)))

#Calculating average rating by each user
ratingsAdded = ratingsWithOne.reduceByKey(lambda l,m: (l[0] + m[0], l[1] + m[1] ))
ratingsAvg = ratingsAdded.mapValues(lambda l: l[0] / l[1])
ratingsAvgMap = ratingsAvg.collectAsMap()

#Broadcasting the average rating by each user
ratingsAvgBroadcast = sc.broadcast(ratingsAvgMap)
...
#Normalizing the ratings by each user, i. e., subtracting the average rating
ratingsNorm = ratings.map(lambda (user,(movie, rating)): (user,(movie, rating - float(ratingsAvgBroadcast.value[user]))))
...
joinedRatings = ratings.join(ratingsNorm)
